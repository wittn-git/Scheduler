...
class Schedule:
    ...
    def draw(self, canvas):
        ...
        for i in range(28):
            if i > 1 and i < 27: canvas.component.create_text(canvas.point(5, i*canvas.max_h/27+2), text='{}.00'.format(i-2), anchor='nw', font = tkFont.Font(family="Times New Roman", size=8))
            if i != 1: canvas.component.create_line(canvas.point(0, i*canvas.max_h/27), canvas.point(canvas.max_w, i*canvas.max_h/27), width=3 if i==2 else 1)
...
class Day:
    ...
    def remove_task(self, task, canvas):
        self.tasks.remove(task).remove_task(canvas)
...
class Task:
    ...
    def draw(self, canvas, frame):
        ...
        self.image = canvas.component.create_rectangle(
            canvas.point(canvas.max_w/8*(self.day_index), canvas.max_h/27*(self.time_from+2)),
            canvas.point(canvas.max_w/8*(self.day_index+1), canvas.max_h/27*(self.time_to+2)),
            fill = self.color,
            tag = self.id
        )
        self.frame = frame
        canvas.component.tag_bind(self.id, '<Button-1>', lambda x: frame.execute_command(6, task=self))

    def remove_task(self, canvas):
        canvas.component.remove(self.image)
